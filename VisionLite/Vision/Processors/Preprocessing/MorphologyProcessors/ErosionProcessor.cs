...
namespace VisionLite.Vision.Processors.Preprocessing.MorphologyProcessors
{
    /// <summary>
    /// 形态学腐蚀处理器
    /// 使用Halcon ErosionCircle和ErosionRectangle1算子实现形态学腐蚀操作
    /// </summary>
    public class ErosionProcessor : VisionProcessorBase
    {
        #region 属性
        ...
        /// 处理器名称
        /// </summary>
        public override string ProcessorName => "形态学腐蚀";
        ...
        /// 结构元素类型
        /// </summary>
        [Parameter("结构元素类型", "选择圆形或矩形结构元素，影响腐蚀的形状特征", Group = "参数设置")]
        public StructuringElementType ElementType { get; set; } = StructuringElementType.Circle;
        
        /// <summary>
        /// 半径
        /// </summary>
        [Parameter("半径", "圆形结构元素的半径，值越大腐蚀效果越强", 0.5, 50.0, DecimalPlaces = 1, Group = "参数设置")]
        ...
        /// 反转输出
        /// </summary>
        [Parameter("反转输出", "是否反转腐蚀结果", Group = "其他设置")]
        ...
        public override async Task<ProcessResult> ProcessAsync(VisionImage inputImage)
        {
            ...
            try
            {
                ...
                // 执行自适应形态学腐蚀
                var outputImage = await Task.Run(() => ExecuteAdaptiveErosion(inputImage));
            ...
            {
                // 返回失败结果
                return CreateFailureResult($"形态学腐蚀处理失败: {ex.Message}", ex);
            }
        }
        ...
        /// <returns>腐蚀处理后的结果</returns>
        private VisionImage ExecuteAdaptiveErosion(VisionImage inputImage)
        {
            ...
            try
            {
                ...
                // 检查输出是否有效
                if (outputImage == null)
                    throw new InvalidOperationException("自适应形态学腐蚀处理失败，输出对象为空");
            ...
            catch (Exception ex)
            {
                // 清理资源
                outputImage?.Dispose();
                throw new InvalidOperationException($"自适应形态学腐蚀操作失败: {ex.Message}", ex);
            }
        }
            ...
            try
            {
                ...
                switch (ElementType)
                {
                    case StructuringElementType.Circle:
                        HOperatorSet.ErosionCircle(regions, out erodedRegions, Radius);
                        algorithm = $"区域腐蚀-圆形(r={Radius})";
                        break;
                        
                    case StructuringElementType.Rectangle:
                        HOperatorSet.ErosionRectangle1(regions, out erodedRegions, Width, Height);
                        algorithm = $"区域腐蚀-矩形({Width}×{Height})";
                        break;
                ...
                // 将腐蚀后的区域转换为二值图像
                HOperatorSet.RegionToBin(erodedRegions, out outputImage, 255, 0, width, height);
                
                erodedRegions?.Dispose();
                return algorithm;
            }
            catch
            {
        ...
        private string ProcessImageInput(HObject image, out HObject outputImage)
        {
            ...
            switch (ElementType)
            {
                case StructuringElementType.Circle:
                    ...
                    HOperatorSet.GrayErosionShape(image, out outputImage, adjustedRadius, adjustedRadius, "octagon");
                    algorithm = adjustedRadius != Radius ? 
                        $"灰度腐蚀-圆形(r={adjustedRadius}，从{Radius}自动调整)" : 
                        $"灰度腐蚀-圆形(r={adjustedRadius})";
                    break;
                ...
                case StructuringElementType.Rectangle:
                    ...
                    HOperatorSet.GrayErosionRect(image, out outputImage, adjustedWidth, adjustedHeight);
                    algorithm = (adjustedWidth != Width || adjustedHeight != Height) ?
                        $"灰度腐蚀-矩形({adjustedWidth}×{adjustedHeight}，从{Width}×{Height}自动调整)" :
                        $"灰度腐蚀-矩形({adjustedWidth}×{adjustedHeight})";
                    break;
        ...
        private void ValidateOutput(HObject output)
        {
            ...
            {
                throw new InvalidOperationException($"形态学腐蚀算子执行异常: {halconEx.Message}", halconEx);
            }
        }
    ...
    {
        /// <summary>圆形结构元素</summary>
        Circle,
        /// <summary>矩形结构元素</summary>
        Rectangle
    }
    ...
    /// 结构元素类型扩展方法
    /// </summary>
    public static class StructuringElementTypeExtensions
    {
        /// <summary>
        /// 获取结构元素类型的中文显示名称
        /// </summary>
        public static string GetDisplayName(this StructuringElementType type)
        {
            return type switch
            {
                StructuringElementType.Circle => "圆形",
                StructuringElementType.Rectangle => "矩形",
                _ => type.ToString()
            };
        }
    }
