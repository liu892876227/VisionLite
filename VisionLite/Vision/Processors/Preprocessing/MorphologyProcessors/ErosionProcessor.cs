...
namespace VisionLite.Vision.Processors.Preprocessing.MorphologyProcessors
{
    ...
    public class ErosionProcessor : VisionProcessorBase
    {
        ...
        /// 结构元素类型
        /// </summary>
        [Parameter("结构元素类型", "选择圆形或矩形结构元素，影响腐蚀的形状特征")]
        public StructuringElementType ElementType { get; set; } = StructuringElementType.Circle;
        
        /// <summary>
        /// 半径
        /// </summary>
        [Parameter("半径", "圆形结构元素的半径，值越大腐蚀效果越强", 0.5, 50.0, DecimalPlaces = 1)]
        ...
        /// 反转输出
        /// </summary>
        [Parameter("反转输出", "是否反转腐蚀结果", Group = "高级设置", IsAdvanced = true)]
        ...
        public override async Task<ProcessResult> ProcessAsync(VisionImage inputImage)
        {
            ...
            try
            {
                ...
                // 执行形态学腐蚀
                var outputImage = await Task.Run(() => ExecuteErosion(inputImage));
        ...
        /// </summary>
        /// <param name="inputImage">输入图像</param>
        /// <returns>腐蚀后的图像</returns>
        private VisionImage ExecuteErosion(VisionImage inputImage)
        {
            ...
            try
            {
                ...
                // 步骤3：将腐蚀后的区域转换为二值图像
                HOperatorSet.RegionToBin(erodedRegions, out outputImage, 255, 0, width.I, height.I);
                ...
                // 检查输出图像是否有效
                if (outputImage == null)
                    throw new InvalidOperationException("形态学腐蚀处理失败，输出图像为空");
            ...
            catch (Exception ex)
            {
                // 清理资源
                outputImage?.Dispose();
                throw new InvalidOperationException($"Halcon形态学腐蚀操作失败: {ex.Message}", ex);
            }
        }
