...
namespace VisionLite.Vision.Processors.Preprocessing.MorphologyProcessors
{
    ...
    public class OpeningProcessor : VisionProcessorBase
    {
        ...
        /// 结构元素类型
        /// </summary>
        [Parameter("结构元素类型", "选择圆形或矩形结构元素，影响开运算的形状特征")]
        public StructuringElementType ElementType { get; set; } = StructuringElementType.Circle;
        
        /// <summary>
        /// 半径
        /// </summary>
        [Parameter("半径", "圆形结构元素的半径，值越大去噪效果越强但细节损失越多", 0.5, 50.0, DecimalPlaces = 1)]
        ...
        /// 反转输出
        /// </summary>
        [Parameter("反转输出", "是否反转开运算结果", Group = "高级设置", IsAdvanced = true)]
        ...
        public override async Task<ProcessResult> ProcessAsync(VisionImage inputImage)
        {
            ...
            try
            {
                ...
                // 执行形态学开运算
                var outputImage = await Task.Run(() => ExecuteOpening(inputImage));
        ...
        private VisionImage ExecuteOpening(VisionImage inputImage)
        {
            ...
            try
            {
                ...
                // 步骤3：将开运算后的区域转换为二值图像
                HOperatorSet.RegionToBin(openedRegions, out outputImage, 255, 0, width.I, height.I);
                ...
                // 检查输出图像是否有效
                if (outputImage == null)
                    throw new InvalidOperationException("形态学开运算处理失败，输出图像为空");
            ...
            catch (Exception ex)
            {
                // 清理资源
                outputImage?.Dispose();
                throw new InvalidOperationException($"Halcon形态学开运算操作失败: {ex.Message}", ex);
            }
        }
