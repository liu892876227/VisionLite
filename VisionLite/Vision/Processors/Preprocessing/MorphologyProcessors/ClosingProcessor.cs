...
namespace VisionLite.Vision.Processors.Preprocessing.MorphologyProcessors
{
    /// <summary>
    /// 形态学闭运算处理器
    /// 使用Halcon ClosingCircle和ClosingRectangle1算子实现形态学闭运算操作
    /// 闭运算 = 先膨胀后腐蚀，用于填充小孔洞、连接断开区域
    /// </summary>
    public class ClosingProcessor : VisionProcessorBase
    {
        #region 属性
        ...
        /// 处理器名称
        /// </summary>
        public override string ProcessorName => "形态学闭运算";
        ...
        public override async Task<ProcessResult> ProcessAsync(VisionImage inputImage)
        {
            ...
            {
                // 返回失败结果
                return CreateFailureResult($"形态学闭运算处理失败: {ex.Message}", ex);
            }
        }
