...
namespace VisionLite.Vision.Processors.Preprocessing.FilterProcessors
{
    ...
    public class MeanFilterProcessor : VisionProcessorBase
    {
        ...
        /// 滤波器大小（奇数）
        /// </summary>
        [Parameter("滤波器大小", "均值滤波器窗口大小，值越大平滑效果越强", 3, 15, Step = 1, DecimalPlaces = 0, Group = "参数设置")]
        ...
        private Dictionary<string, object> CreateMeasurements(VisionImage inputImage, TimeSpan processingTime)
        {
            ...
            measurements["MeanImage实际输出"] = string.IsNullOrEmpty(_processedImageType) ? "未检测" : _processedImageType;
            ...
            {
                measurements["转换状态"] = $"保持MeanImage输出类型：{_processedImageType}";
            }
            else
            {
