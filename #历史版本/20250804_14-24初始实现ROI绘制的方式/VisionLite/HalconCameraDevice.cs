...
namespace VisionLite
{
    ...
    public class HalconCameraDevice : ICameraDevice
    {
        ...
        public event Action<HObject> ImageAcquired; // 实现接口定义的事件
        ...
        public void GrabAndDisplay()
        {
            ...
            try
            {
                ...
                {
                    // 在显示前，先将图像复制一份并赋值给 m_Ho_Image
                    m_Ho_Image?.Dispose();
                    m_Ho_Image = tempImage.CopyObj(1, -1);

                    // 触发事件，将新图像上报给订阅者 (MainWindow)
                    ImageAcquired?.Invoke(m_Ho_Image);

                    // 在自己的窗口中显示
                    Display(); // Display 方法现在只负责显示 m_Ho_Image
                }
